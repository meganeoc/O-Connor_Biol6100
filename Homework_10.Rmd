...
Generate_Data <- function(seed=200, n=100, groupmeans = c(10, 40, 50)) {
  set.seed(seed)
  groupSD <- runif(length(groupmeans))
  
  group <- rep(1:length(groupmeans), each = n)
  response <- c(rnorm(n, mean = groupmean[1]),
             rnorm(n, mean = groupmean[2]),
             rnorm(n, mean = groupmean[3]))
  RandomData <- data.frame(group, response)
...
```{r}
reshuffle_mean_calc <- function(RandomData) {
